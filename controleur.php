		...
		switch($action)
		{
			...
			case 'Parrainage':
				...
				if($pseudo=valider("pseudo","SESSION")){
					// on conserve l'email dans une colonne SQL
					// on fait appel, donc, à une nouvelle fonction de SQL dans modele
					ajoutFilleul($email,$pseudo);
				}

				$qs="?view=parrainage";
					break;
			case "Verfication du parrainage":
				$OK=false;
				if ($email=verifier("pseudo"))
				if ($pseudo=verifier("pseudo")){
					if (verifierEmail($email,$pseudo)){
						$OK=true;
					}
