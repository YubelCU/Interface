...
function ajoutFilleul($email,$pseudo)
{

	// on ajoute la valeur du mail au pseudo rattaché

	$SQL="update users set emailfilleul = '$email' where pseudo = '$pseudo' ";
	SQLUpdate($SQL);

}

function verifierEmail($email,$pseudo){
	$SQL= "select emailfilleul from users where pseudo='$pseudo'";
	return ($email==SQLGetChamp($SQL));
